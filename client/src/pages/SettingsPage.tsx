...
export default function SettingsPage(): React.ReactElement {
  ...
  const [mapsApiKey, setMapsApiKey] = useState<string>(user?.maps_api_key || '')
  ...
  const saveMapSettings = async (): Promise<void> => {
    ...
    try {
      ...
      if ((mapsApiKey || '') !== (user?.maps_api_key || '')) {
        await updateMapsKey(mapsApiKey || null)
      }
          ...
          <Section title={t('settings.map')} icon={Map}>
            ...
            <div>
              ...
              <input
                type="password"
                value={mapsApiKey}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMapsApiKey(e.target.value)}
